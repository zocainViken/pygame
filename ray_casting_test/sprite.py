...
SCALE = WIDTH // NUM_RAY
FAKE_RAYS = 100
FAKE_RAYS_RANGE = NUM_RAY - 1 + 2 * FAKE_RAYS
...
class Sprites:
    def __init__(self):
        self.sprite_parameters = {
            'sprite_barrel': {
                ...
                'death_animation': deque(
                    [pygame.image.load(f'sprites/barrel/death/{i}.png').convert_alpha() for i in range(3)]),
                'is_dead' : None,
                'dead_shift' : 2.6,
                'animation_dist': 800,
                'animation_speed': 10,
            ...
            'sprite_devil': {
                'sprite': [pygame.image.load(f'sprites/devil/base/{i}.png').convert_alpha() for i in range(7)],
                'viewing_angles': True,
                'shift': -0.2,
                #'scale': 1.1,
                'side': 50,
                'scale': (1.1, 1.1),
                'animation': deque(
                    [pygame.image.load(f'sprites/devil/anim/{i}.png').convert_alpha() for i in range(8)]),
                'death_animation': deque(
                    [pygame.image.load(f'sprites/devil/death/{i}.png').convert_alpha() for i in range(5)]),
                'is_dead' : None,
                'dead_shift' : 0.6,
                'animation_dist': 800,
                'animation_speed': 10,
        ...
        self.list_of_objects = [
            SpriteObject(self.sprite_parameters['sprite_barrel'], (1.1, 1.1)),#top left mini map
            SpriteObject(self.sprite_parameters['sprite_barrel'], (21.9, 1.1)),#top right mini map
            SpriteObject(self.sprite_parameters['sprite_barrel'], (21.9, 13.9)),#bottom right mini map
            SpriteObject(self.sprite_parameters['sprite_barrel'], (1.1, 13.9)),#bottom left mini map
            ...
            SpriteObject(self.sprite_parameters['sprite_door_v'], (7, 4)),

            SpriteObject(self.sprite_parameters['sprite_door_h'], (8.6, 5.6)),
...
class SpriteObject:
    def __init__(self, parameters, pos):
        ...
        self.delete = False
        self.open_door_trigger = False
        self.previous_door_position = self.y if self.flag == 'door_h' else self.x
        self.delete = False
        if self.viewing_angles:
            print(len(self.object))
            if len(self.object) == 8:
                #self.sprite_angles = [frozenset(range(i, i + 45)) for i in range(0, 360, 45)]
                self.sprite_angles = [frozenset(range(338, 361)) | frozenset(range(0, 23))] + [frozenset(range(i, i + 45)) for i in range(23, 338, 45)]
            else:
                #self.sprite_angles = [frozenset(range(i, i + 45)) for i in range(0, 360, 45)]
                self.sprite_angles = [frozenset(range(348, 361)) | frozenset(range(0, 11))] + [frozenset(range(i, i + 23)) for i in range(11, 348, 23)]
